...
> 后续演化：基础导入由 V16 落地；AI 重建与生命周期治理分别见 V1.1/V17 和 V1.2/V18。V1.2 不改变本文件中的 20 MiB、Preview 后创建项目、编码与临时源清理边界。
...
`TxtChapterParser` 的真实版本号为 `txt-lines-v2`。只有独立行、不超过 120 字符且完整匹配 heading pattern 时才切分。v2 在匹配前执行 Unicode NFKC 兼容规范化，因此可识别网文 TXT 常见的全角空格（U+3000）和全角数字，同时预览与正式章节名仍保留原始字符。
